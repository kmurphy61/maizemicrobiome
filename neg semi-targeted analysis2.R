...
intens3 <- read.table(file = "peak_height_neg_2020.txt" , header = TRUE)
intens3$ID <- as.character(intens3$ID)
intens3$mz <- as.character(intens3$mz)
intens3$rt <- as.character(intens3$rt)

var_match <- read.table(file = "variable with name neg 2020.txt", header = T)
...
metab_plot
  ggsave(filename = "pcoa_metab_neg.jpg", path = NULL,
...
comp.lm_results_ND_export_neg <- comp.lm_results_ND %>% 
  filter(p.value <= 0.05) %>%
  arrange(estimate) %>% 
...
comp.lm_results_ND_export_neg <- comp.lm_results_ND_export_neg %>%
  mutate(mode = "negative")%>%
  mutate(key = paste(mode, variable, sep = "-"))

write.table(comp.lm_results_ND_export_neg, "/Users/zerbe/Desktop/untargeted/sig diff lists/NEG/comp.lm_results_ND_export.txt", sep="\t")

head(comp.lm_results_ND_export_neg)
head(var_match)

#enriched in WT
AN2 <-  comp.lm_results_ND_export_neg %>% 
  filter(p.value <= 0.05) %>% 
  filter(estimate > 0)

#encriched in AN2
WT <-  comp.lm_results_ND_export_neg %>% 
  filter(p.value <= 0.05) %>% 
  filter(estimate < 0)
